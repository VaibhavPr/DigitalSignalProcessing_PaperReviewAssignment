...
New_Img(:,:,1) = ProposedMethod(Img(:,:,1),2);
New_Img(:,:,2) = ProposedMethod(Img(:,:,2),2);
New_Img(:,:,3) = ProposedMethod(Img(:,:,3),2);
...
subplot(2,2,2);imshow(New_Img);title('Proposed');
%subplot(2,3,4);imshow(New_Img);title('Proposed method-W/O LPF');
subplot(2,2,4);imhist(New_Img(:,:,3));title('Equalized Histogram-W/O LPF');
